...
`DeterministicBacktest` only exposes point-in-time non-revised bars to signals. `OrderIntent` requires ordered information, signal, eligibility, and submission timestamps. `FillModel` requires an explicit fidelity tier, latency, participation bound, and queue assumption where relevant; it refuses unresolved L2 sequence gaps and never treats a touched limit as a guaranteed fill. Every simulation is `BITWISE` reproducible unless a caller explicitly runs a separate stochastic model.
