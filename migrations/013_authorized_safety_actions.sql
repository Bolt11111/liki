DROP POLICY artifact_append ON artifacts;
CREATE POLICY artifact_append ON artifacts FOR INSERT TO liki_app WITH CHECK
  (liki_owns(producer_id) AND (classification<>'SEALED' OR liki_has_role(ARRAY['sealed_evaluator'])) AND
   (liki_has_role(ARRAY['research','evaluator','sealed_evaluator','data','paper','inference','governance']) OR
    (liki_has_role(ARRAY['owner']) AND schema_name='governance/server-owner-decision-attestation/v1')));

CREATE POLICY emergency_schedule_insert ON governance_schedules FOR INSERT TO liki_app WITH CHECK
  (cadence='EMERGENCY' AND liki_has_role(ARRAY['scheduler','owner','operator','paper']));
CREATE POLICY emergency_schedule_event ON event_log FOR INSERT TO liki_app WITH CHECK
  (aggregate_type='governance_schedule' AND event_type='GOVERNANCE_EMERGENCY_SCHEDULED'
   AND liki_owns(principal_id) AND liki_has_role(ARRAY['scheduler','owner','operator','paper']));
CREATE POLICY emergency_schedule_aggregate ON aggregates FOR INSERT TO liki_app WITH CHECK
  (aggregate_type='governance_schedule' AND state->>'status'='DUE'
   AND liki_has_role(ARRAY['scheduler','owner','operator','paper']));
