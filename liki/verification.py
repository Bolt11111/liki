...
VERIFIER_VERSION = "deterministic-gates-v4"
    ...
    def execute(
        ...
        with self.store.transaction(credential) as (conn, actor):
            ...
            conn.execute(
                ...
                "'DEVELOPMENT',now(),'{}',%s,now())",
                (evidence_id, report_id, EVIDENCE_CLASSES[gate_id], actor.principal_id),
            )
            return evidence_id
